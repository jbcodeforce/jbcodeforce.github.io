...
- kinematics  describes the way objects move—their velocity and their acceleration 
- Position relative to a reference frame
- Displacement is the change in position of an object = Xf - X0. Displacement is defined by both direction and magnitude: a vector quantity
- Distance is the magnitude or size of displacement between two positions. it has no sign
...
- time is related to motion by looking at elapsed time for a particular motion. Δ t = tf - t0
- Average velocity is displacement (change in position) divided by the time of travel v = Δ X / Δ t. When arriving at the same position as  the beginning: xf = x0 then average velocity is zero
...
- x = x0 + v . t : gives insight into the relationship between displacement, average velocity, and time
- v = v0 + a.t : gives us insight into the relationships among velocity, acceleration, and time
- v^2 = v0^2 + 2a(x − x0 ) 

![](https://latex.codecogs.com/svg.image?v^{2}%20=%20v_{0}^{2}%20+%202.a.(X%20-%20X_{0}))
 

## Problem solving

- Examine the situation to determine which physical principles are involved. uses sketch
...
- Falling objects, if air resistance and friction are negligible, fall toward the center of Earth with the same constant acceleration, independent of their mass. a = 1.67m/s^2
- Air resistance opposes the motion of an object through the air, while friction between objects
- acceleration due to gravity g = 9.8 m/s^2. g varies from 9.78 m/s2 to 9.83 m/s2 , depending on latitude, altitude, underlying geological formations, and local topography
...
- Inertia is the tendency of an object to remain at rest or remain in motion. Inertia is related to an object’s mass. Any change in velocity (speed or direction) must be caused by an external force 
- The inertia of an object is measured by its mass. 
- Mass is the quantity of matter in a substance. 
- Unlike weight, mass does not vary with location, it  is the same on Earth, in orbit, or on the surface of the Moon. 
- Change in motion is equivalent to a change in velocity -> A change in velocity means, by definition, that there is an acceleration acting on the objet.
...
- **Newton’s second law of motion** is  `a= Fnet / m`  or `Fnet = a . m`. The unit is the Newton. `1 N = 1 kg . m/s^2`  is the force needed to accelerate a `1-kg` system at the rate of `1m/s2`  
- This is the equation for weight— The gravitational force on a mass m : `w = mg`.  the weight of an object is the gravitational force on it from the nearest large body.
...
- Normal force is perpendicular to the surface of the contact and  is supporting the load of the weight applied to this surface
...
Once the applied force exceeds `fs(max)`, the object will move. Then the friction is `fk = μkN`, 
...
The magnitude is `a = V^2` and `r = r ω ^ 2`
...
- At the start of a roller coaster its PE is max and it will lose it as it goes downhill. The net work on the roller is the gravity so:  
...
The conservation of mechanical energy :  `KEi + PEi = KEf + PEf` for conservative force only.

## Non conservative forces

- A non-conservative force is one for which work depends on the path taken. The work done by a non-conservative force adds or removes mechanical energy from a system. 
